...
def read_one_h5(filename: str, data_class: str) -> tuple:
    ...
    D = 2
    buoyancy_val = jax.device_put(
        jnp.array(data_dict["buo_y"][()]), jax.devices("cpu")[0]
    )  # (100,)
    buoyancy_field = jnp.tensordot(buoyancy_val, jnp.ones((N,) * D), axes=0)  # (100,128,128)
    buoyancy = jnp.stack(
        [jnp.zeros(buoyancy_field.shape), buoyancy_field], axis=-1
    )  # (100,128,128,2)
...
def merge_h5s_into_layer(
    ...
    """
    all_files = sorted(
        filter(lambda file: f"NavierStokes2D_{data_class}" in file, os.listdir(data_dir))
    )
...
def get_data(
    ...
    test_single_X, test_single_Y = merge_h5s_into_layer(
        data_dir, num_test_traj, "test", past_steps, 1
    )
    test_rollout_X, test_rollout_Y = merge_h5s_into_layer(
        data_dir, num_test_traj, "test", past_steps, rollout_steps
    )
...
def plot_layer(test_layer, actual_layer, save_loc, future_steps):
    test_images = [
        geom.GeometricImage(img, 0, test_layer.D, test_layer.is_torus)
        for img in test_layer[(1, 0)][0, ..., 0]
    ]
    actual_images = [
        geom.GeometricImage(img, 0, test_layer.D, test_layer.is_torus)
        for img in actual_layer[(1, 0)][0, ..., 0]
    ]
    ...
    for col in range(future_steps):
        ...
        utils.plot_image(
            (actual_images[col] - test_images[col]).norm(),
            ax=axes[2, col],
            title=f"{col}",
            colorbar=True,
        )
...
def map_and_loss(
    ...
    for _ in range(future_steps):
        ...
        layer_x, out_layer = ml.autoregressive_step(
            layer_x, learned_x, out_layer, past_steps, {(1, 0): 1}
        )
...
def train_and_eval(
    ...
    if images_dir is not None:
        ...
        plot_layer(
            ...
            test_rollout_Y.get_one(),
            images_dir + f"/{model_name}_rollout.png",
            5,
        )
