...
def batch_net_old(params, layer, key, train, conv_filters, return_params=False):
    ...
    layer, params = batch_conv_layer(
        ...
        (
            target_k,
            target_parity,
        ),  # final_layer, make sure out output is target_img shaped
        None,
        return_params,  # mold_params
        None,
        None,
...
def baseline_net(params, layer, key, train, return_params=False):
    ...
    reshaped_block = transposed_block.reshape(
        (transposed_block.shape[0],) + transposed_block.shape[2:]
    )
    ...
    layer = geom.BatchLayer(
        {(1, 0): jnp.expand_dims(jnp.moveaxis(layer[(0, 0)], 1, -1), 1)},
        layer.D,
        layer.is_torus,
    )
    ...
    params, train_loss, val_loss = ml.train(
        ...
        optimizer=optax.adam(
            optax.exponential_decay(
                lr, transition_steps=int(X_train.L / batch_size), decay_rate=0.995
            )
        ),
        validation_X=X_val,
        validation_Y=Y_val,
...
def handleArgs(argv):
    ...
    parser.add_argument(
        "-n",
        "--num_trials",
        help="number of runs in the benchmark",
        type=int,
        default=5,
    )
    parser.add_argument("-seed", help="the random number seed", type=int, default=None)
    parser.add_argument(
        "-s", "--save", help="file name to save loss values", type=str, default=None
    )
    parser.add_argument(
        "-l", "--load", help="file name to load loss values", type=str, default=None
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
conv_filters = geom.get_invariant_filters(
    Ms=[3], ks=[0, 1, 2, 3, 4], parities=[0, 1], D=D, operators=group_actions
)
conv_filters_old = geom.get_invariant_filters(
    Ms=[3], ks=[1, 2], parities=[0], D=D, operators=group_actions
)
