...
def plot_results(params, layer_x, layer_y, axs, titles, conv_filters):
...
def batch_net(params, layer, key, train, conv_filters, return_params=False):
    ...
    for dilation in [1, 2, 4, 2, 1, 1, 2, 1]:  # dilated layers in sequence
        ...
        layer, params = ml.VN_nonlinear(
            params, layer, scalar_activation=jax.nn.tanh, mold_params=return_params
        )
...
def handleArgs(argv):
    ...
    parser.add_argument(
        "-d", "--decay", help="decay rate of learning rate", type=float, default=0.995
    )
...
warmup_steps = (
    1  # number of initial steps to run, ensures that particles don't start directly adjacent.
)
...
validation_X, validation_Y = get_data(
    N,
    D,
    num_points,
    num_steps,
...
if load_file:
    ...
    optimizer = optax.adam(
        optax.exponential_decay(
            lr, transition_steps=int(train_X.get_L() / batch_size), decay_rate=decay
        )
    )
...
plot_results(dict(params), train_X.get_one(), train_Y.get_one(), axs[0], titles, conv_filters)
plot_results(dict(params), test_X.get_one(), test_Y.get_one(), axs[1], titles, conv_filters)
plt.savefig(outfile)

print(
    "Full Test loss:",
    map_and_loss(dict(params), test_X, test_Y, None, None, conv_filters=conv_filters),
)
