...
def map_and_loss(params, layer_x, layer_y, key, train, net_f):
    ...
    return jnp.mean(jax.vmap(ml.l2_loss)(recon_img, y_img)) # calculate loss, then average over batch
...
key, subkey = random.split(key)
test_loss = map_and_loss(params, test_scalar_layer, test_grad_layer, subkey, train=False, net_f=div_net_f)
print(f'test loss: {test_loss}')

fig, axs = plt.subplots(nrows=2, ncols=4, figsize=(32,12))

train_x_img = geom.GeometricImage(train_scalar_layer[(0,0)][0,0,2:-2,2:-2], 0, D, is_torus=False)
train_y_img = geom.GeometricImage(train_grad_layer[(1,0)][0,0,2:-2,2:-2], 0, D, is_torus=False)
train_recon_img = geom.GeometricImage(
    div_net_f(params, train_scalar_layer.get_subset(jnp.array([0])), subkey, False)[(1,0)][0,0,2:-2,2:-2],
    0, 
    D, 
    is_torus=False,
)

utils.plot_image(train_x_img, ax=axs[0,0], vmin=None, vmax=None, colorbar=True, title='Train Scalar Field')
...
test_recon_img = geom.GeometricImage(
    div_net_f(params, test_scalar_layer.get_subset(jnp.array([0])), subkey, False)[(1,0)][0,0,2:-2,2:-2],
    0, 
    D, 
    is_torus=False,
)

utils.plot_image(test_x_img, ax=axs[1,0], vmin=None, vmax=None, colorbar=True, title='Test Scalar Field')
