...
def classify_eval(z_train, Y_eval_train, z_test, Y_eval_test):
    ...
    train_accuracy = classification_report(Y_eval_train, clf.predict(z_train), output_dict=True)[
        "accuracy"
    ]
    test_accuracy = classification_report(Y_eval_test, clf.predict(z_test), output_dict=True)[
        "accuracy"
    ]
...
def train_and_eval(data, rand_key, model_name, net, batch_size, lr, epochs, verbose):
    ...
    for X_eval_train, X_eval_test, Y_eval_train, Y_eval_test in eval_data:
        ...
        rand_key, subkey = random.split(rand_key)
        z_train_list = ml.map_in_batches(
            embedding_net,
            params,
            X_eval_train,
            batch_size,
        ...
        z_train = reduce(
            lambda carry, val: jnp.concatenate(
                [carry, val.reshape((-1, embedding_d))]
            ),  # (gpus,batch/gpus,embedding_d)
            z_train_list,
            jnp.ones((0, embedding_d)),
        )

        rand_key, subkey = random.split(rand_key)
        z_test_list = ml.map_in_batches(
            embedding_net,
            params,
            X_eval_test,
            batch_size,
...
conv_filters = geom.get_invariant_filters(
    Ms=[3], ks=[0, 1, 2, 3], parities=[0], D=D, operators=operators
)
...
for test_dataset in [
    "linear",
    "conservative_vs_nonconservative",
    "incompressible_vs_compressible",
]:
    test_data_path = f"../phase2vec/output/data/{test_dataset}"
    X_eval_train, X_eval_test, Y_eval_train, Y_eval_test, _, _ = p2v_models.load_dataset(
        D, test_data_path
    )
    eval_data.append((X_eval_train, X_eval_test, Y_eval_train, Y_eval_test))

# generate function library
ode_basis = p2v_models.get_ode_basis(D, N, [-1.0, -1.0], [1.0, 1.0], 3)
...
models = [
    ...
    (
        "lasso",
        partial(eval_baseline, func=partial(p2v_models.lasso, library=ode_basis)),
    ),
]
