...
def batch_net(params, layer, key, train, conv_filters, return_params=False):
    ...
    layer, params = ml.batch_conv_layer(
        params, layer, conv_filters, depth=1, mold_params=return_params
    )
...
def map_and_loss(
    ...
    return jnp.mean(
        vmap(ml.rmse_loss)(batch_net(params, x, key, train, conv_filters)[(1, 0)], y[(1, 0)])
    )
...
def handleArgs(argv):
    ...
    parser.add_argument(
        "-v",
        "--verbose",
        help="how many status messages to print during training",
        type=int,
        default=1,
    )
...
key, subkey = random.split(key)
X_test = [
    geom.GeometricImage(data, 0, D)
    for data in random.normal(subkey, shape=((num_images,) + (N,) * D + (D,) * k))
]
...
print(
    "test loss:",
    map_and_loss(params, layer_x_test, layer_y_test, None, None, filter_layer),
)
